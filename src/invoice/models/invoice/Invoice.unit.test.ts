...
test('issuing stamps the five fields the system decides', () => {
  ...
  invoice.applyIssue({ prefix: 'FE', number: 1247, issuedAt: at, issuer: {} })
...
test('the issued number overwrites the one somebody had typed', () => {
  ...
  invoice.applyIssue({ prefix: 'FE', number: 1247, issuedAt: Date.now(), issuer: {} })
...
test('issuing writes the number without losing its neighbours in the record', () => {
  ...
  invoice.applyIssue({ prefix: 'FE', number: 7, issuedAt: Date.now(), issuer: {} })
...
test('issuing a document with an empty prefix keeps the bare consecutive', () => {
  ...
  invoice.applyIssue({ prefix: '', number: 42, issuedAt: Date.now(), issuer: {} })
...
test('issuing refuses a number that is not a whole consecutive', () => {
  ...
  assert.throws(() =>
    invoice.applyIssue({ prefix: 'FE', number: 1.5, issuedAt: Date.now(), issuer: {} }),
  )
})
