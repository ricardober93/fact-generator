# template-presets Specification

## Purpose

El catálogo de diseños de partida: qué declara un preset, la garantía de que
produce un documento válido y autosuficiente, cómo se previsualiza en la galería
y qué significa aplicarlo a una plantilla nueva o ya existente. No cubre el modelo
del documento ni cómo se pinta: eso es `invoice-document-model` e
`invoice-renderer`; ni la edición en sí, que es `template-editor`.
